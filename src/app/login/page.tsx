...
export default function LoginPage() {
  ...
  const submit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try { const r = await fetch("/api/auth/login", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) }); const j = await r.json(); if (r.ok) router.push("/dashboard"); else { setError(j.error||"Failed"); setStatus("error"); } } catch { setError("Error"); setStatus("error"); }
  };
    ...
    <div className="min-h-screen flex items-center justify-center px-6 py-32">
      ...
      <AnimateIn variant="scale" className="relative z-10 w-full max-w-sm">
        <div className="text-center mb-10">
          ...
          <h1 className="text-2xl type-heading text-bone mb-1">Welcome back</h1>
          <p className="text-[13px] text-bone-ghost">Sign in to continue</p>
        </div>
        <form onSubmit={submit} className="space-y-5">
          ...
          <div><label htmlFor="password" className="type-caption text-bone-ghost mb-2 block">Password</label>
            <div className="relative"><input id="password" name="password" type={showPw?"text":"password"} required placeholder="••••••••" className="w-full px-4 py-3 rounded-xl bg-void-card border border-bone-faint/10 text-[14px] text-bone placeholder:text-bone-faint focus:outline-none focus:border-lux-border transition-colors pr-14" />
          ...
          <button type="submit" disabled={status==="loading"} className="w-full text-[11px] font-semibold tracking-wide uppercase bg-lux text-void py-3 rounded-full hover:bg-lux/90 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
            {status==="loading" && <span className="w-4 h-4 border-2 border-void/30 border-t-void rounded-full animate-spin" />}Sign in
          </button>
        </form>
        <p className="text-center text-[13px] text-bone-ghost mt-8">No account? <Link href="/signup" className="text-lux hover:underline underline-offset-4 font-medium">Sign up</Link></p>
      </AnimateIn>
    </div>
  );
}
